...
export default function SafeWalkPage({ userInfo, monitor, isSubComponent }) {
    ...
    try {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          async (pos) => {
            await startSafeWalk(userInfo?.user_id || "anonymous", userInfo?.name || "Anonymous", dest, etaMinutes, pos.coords.latitude, pos.coords.longitude, userInfo?.emergency_contact || "");
          },
          async () => {
  ...
  const handleCancel = async () => {
    try { await cancelSafeWalk(userInfo?.user_id || "anonymous"); } catch (err) {}
    globalConfirmSafe();
  };

  const handleExtend = async () => {
    try { await extendSafeWalk(userInfo?.user_id || "anonymous", 10); } catch (err) {}
      ...
      {!sw.active ? (
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                <input
                  type="range" min="1" max="120" value={etaMinutes}
            ...
            <button onClick={handleStart} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-xl shadow-lg shadow-emerald-600/30 transition-colors mt-4 text-lg">
              Start Safe Walk
            </button>
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-emerald-500/50 rounded-2xl p-6 mt-4 shadow-2xl text-center flex flex-col items-center">
          ...
          <div className="grid grid-cols-2 gap-4 w-full">
            <button onClick={handleExtend} className="bg-slate-800 hover:bg-slate-700 border border-slate-600 text-white font-semibold py-3 rounded-xl transition-colors">
              +10 Mins
            </button>
            <button onClick={handleCancel} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl transition-colors">
              I'm Safe
            </button>
          </div>
        </div>
